  ...
  const handlePublish = async () => {
    try {
      ...
      const uploadFile = async (file: File, folder: string) => {
        ...
        return data.publicUrl; // ✅ this is safe, no urlError
      };
        
      const videoUrl = videoFile ? await uploadFile(videoFile, "videos") : "";
      const pdfUrl = pdfFile ? await uploadFile(pdfFile, "pdfs") : "";
      const thumbnailUrl = thumbnailFile ? await uploadFile(thumbnailFile, "thumbnails") : "";
  
      // Save course metadata in Supabase Postgres
      ...
      toast({ title: "Lesson Published!", description: "Your lesson is now live." });
      navigate("/instructor/dashboard");
    } catch (err: any) {
      console.error(err);
      toast({ title: "Error", description: err.message || "Failed to publish lesson." });
    }
  };
          ...
          {step === 2 && (
            <Card className="p-8">
              ...
              <div className="space-y-6">
                ...
                <div
                  className="mt-2 border-2 border-dashed border-border rounded-lg p-12 text-center hover:border-primary transition-colors cursor-pointer"
            ...
            <Card className="p-8">
              ...
              <div className="space-y-6 max-h-[400px] overflow-y-auto pr-2">
                ...
                <div className="border border-border rounded-lg p-4 hover:border-primary transition-colors">
                  <Label className="mb-2">Lesson Video</Label>
                  <input
                    type="file"
                    accept="video/*"
                  ...
                  />
                  {videoFile && (
                    <p className="mt-2 text-sm text-primary font-medium">{videoFile.name}</p>
                  )}
                </div>

                {/* Lesson PDF */}
                <div className="border border-border rounded-lg p-4 hover:border-primary transition-colors">
                  <Label className="mb-2">Lesson PDF</Label>
                  <input
                    type="file"
                  ...
                  />
                  {pdfFile && (
                    <p className="mt-2 text-sm text-primary font-medium">{pdfFile.name}</p>
                  )}
                </div>

                {/* Mini Project / Exercise */}
                <div className="border border-border rounded-lg p-4 hover:border-primary transition-colors">
                  <Label className="mb-2">Mini Project / Exercise</Label>
                  <Textarea
                    value={miniProject}
                    onChange={(e) => setMiniProject(e.target.value)}
                    placeholder="Describe mini project question or assignment"
                    className="mt-2"
                  />
                </div>
              </div>
